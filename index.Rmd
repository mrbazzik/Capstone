...
setwd("c:/Users/Administrator/Documents/GitHub/Capstone/")
...
get_prop_data <- function(vec, thres){
  total_sum<-sum(vec)
  cum_sum <- 0
  cover <- c()
  for(name in names(vec)){
    cum_sum <- cum_sum + vec[name]
    prop <- cum_sum/total_sum
    cover <- c(cover,prop)
    if (prop>=thres) break
  }
  names(cover)<-names(vec)[1:length(cover)]  
}

uni_Freq<-row_sums(uni_tdm)
uni_Freq<-sort(uni_Freq, decreasing=T)
head(uni_Freq)
total_sum<-sum(uni_Freq)
cum_sum <- 0
cover <- c()
for(name in names(uni_Freq)){
  cum_sum <- cum_sum + uni_Freq[name]
  cover <- c(cover,cum_sum/total_sum)
}
names(cover)<-names(uni_Freq)
...
bi_tdmFreq<-removeSparseTerms(bi_tdm, 0.995)
dim(bi_tdmFreq)

dim(tri_tdm)
inspect(tri_tdm[1:10,1:10])
...
#      ylab("")+
#      ggtitle("Distribution of unigrams among documents")
# 
...
#      scale_fill_gradient(high="#FF0000" , low="#FFFFFF")+
#      ylab("")+ 
#      ggtitle("Distribution of bigrams among documents")
# 
...
#      scale_fill_gradient(high="#FF0000" , low="#FFFFFF")+
#      ylab("")+
#      ggtitle("Distribution of trigrams among documents")
#      
